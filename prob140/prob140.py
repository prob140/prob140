...
def check_valid_probability_table(table):
    """
    Checks whether or not the table provided is a valid Probability Table
    """
...
def plot_dist(self, width=1, mask=[], **vargs):
    ...
    if len(mask) == 0:
        ...
        if isinstance(mask[0], collections.Iterable):
            ...
            plt.xlabel("Domain")
            plt.ylabel("Percent per unit")
            # dist1 = Table().domain(domain[mask]).probability(prob[mask])
            # dist2 = Table().domain(domain[np.logical_not(mask)]).probability(prob[np.logical_not(mask)])
...
def domain(self, values):
    """
    Assigns domain values to a Table
...
def probability_function(self, pfunc):
    """
    Assigns probabilities to a Table via a probability
    ...
    pfunc : univariate function
        Probability function of the Table

    Returns
    -------
    Table
        Table with those probabilities
...
def sd(self):
    """
    Finds standard deviation of Table
